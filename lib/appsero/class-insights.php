...
/**
 * MoreConvert\McCompare\Appsero Insights
 *
...
class Insights {
	...
	/**
	 * MoreConvert\McCompare\Appsero\Client
	 *
	 * @var object
	 */
	protected $client;
	...
	public function init_plugin() {
		...
		add_action( 'admin_print_styles-plugins.php', array( $this, 'deactivation_modal_styles' ) );
		add_action( 'admin_print_scripts-plugins.php', array( $this, 'deactivation_modal_scripts' ) );
		...
		if ( $this->show_notice ) {
			...
			add_action(
				...
				function () {
					...
					$inline_js = "
                    jQuery(function($) {
                        $('." . esc_js( $this->client->slug ) . "-insights-data-we-collect').on('click', function(e) {
                            e.preventDefault();
                            $(this).closest('.updated').find('p.description').slideToggle('fast');
                        });
                    });
                ";
	...
	private function is_optin_request() {
		return isset( $_GET[ $this->client->slug . '_tracker_optin' ] ) && 'true' === $_GET[ $this->client->slug . '_tracker_optin' ];// phpcs:ignore
	}
	...
	private function is_optout_request() {
		return isset( $_GET[ $this->client->slug . '_tracker_optout' ] ) && 'true' === $_GET[ $this->client->slug . '_tracker_optout' ];// phpcs:ignore
	}
	...
	public function get_post_count( $post_type ) {
		...
		return (int) $wpdb->get_var( // phpcs:ignore
			$wpdb->prepare(
				"SELECT count(ID) FROM $wpdb->posts WHERE post_type = %s and post_status = %s", // phpcs:ignore
				$post_type,
				'publish'
			)
		);
	...
	public function add_weekly_schedule( $schedules ) {
		$schedules['weekly'] = array(
			...
			'display'  => __( 'Once Weekly', 'moreconvert-compare-for-woocommerce' ),
		);
	 ...
	 */
	public function deactivation_modal_styles() {
	...
	/**
	 * Deactivation modal scripts
	 */
	public function deactivation_modal_scripts() {
		wp_enqueue_script( 'jquery' );

		$custom_js = "
			(function($) {
				$(function() {
					var modal = $('#" . esc_js( $this->client->slug ) . "-wd-dr-modal');
					var deactivateLink = '';

					// Open modal
					$('#the-list').on('click', 'a." . esc_js( $this->client->slug ) . "-deactivate-link', function(e) {
						e.preventDefault();
