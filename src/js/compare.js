...
(function ($, document) {
	 ...
	 */
	const CompareManager = function ($element) {
		// Properties
	...
	CompareManager.prototype.init = function () {
		...
		if (typeof wp === 'undefined' || typeof wp.template !== 'function') {
			log('wp.template is not available. Ensure wp-util is enqueued.');
			return false;
		}
		if (
			!document.getElementById('tmpl-moreconvert-compare-popup-template')
		) {
			log(
				'Template #tmpl-moreconvert-compare-popup-template not found in DOM.'
			);
			return false;
		}
		if (
			!document.getElementById(
				'tmpl-moreconvert-compare-add-product-popup-template'
			)
		) {
			log(
				'Template #tmpl-moreconvert-compare-add-product-popup-template not found in DOM.'
			);
			return false;
		}
		if (
		...
		// Templates
		this.comparePopupTemplate = wp.template(
			'moreconvert-compare-popup-template'
		);
		this.addProductPopupTemplate = wp.template(
			'moreconvert-compare-add-product-popup-template'
		);
	...
	CompareManager.prototype.bindEvents = function () {
		...
		$(document.body).on(
			'click',
			'.moreconvert-compare-reset-variations',
			(function (_this) {
				return function (e) {
					e.preventDefault();
					...
					$(this)
						...
						.each(function () {
							const index = $(this)
								.closest('.moreconvert-compare-product-card')
								.data('index');
							if (_this.compareProducts[index]) {
								_this.resetProduct(
									_this.compareProducts[index]
								);
		...
		$(document).on(
			...
			(function (_this) {
				return function (e) {
					...
					if (!productData) {
						log(
							'No product data found for .moreconvert-compare-button'
						);
						return;
					}
						...
						$.ajax({
							...
							success(response) {
								if (
									...
									productData = _this.processProductData(
										productData,
										response.data
									);
									_this.allCompareProducts = [productData];
									_this.compareProducts =
										_this.allCompareProducts.slice(
											0,
											_this.getMaxDisplayed()
										);
							...
							error(jqXHR, textStatus) {
								log(
									'AJAX error fetching variations:',
									textStatus
								);
								//_this.initializeComparison(productData);
							},
		...
		$(document).on(
			'click',
			'.moreconvert-compare-add-to-compare-button, .moreconvert-compare-product-link',
			(function (_this) {
				return function (e) {
					e.preventDefault();
					let productData = $(this).data('product');
					if (!productData) {
						log(
							'No product data found for .moreconvert-compare-add-to-compare-button or .moreconvert-compare-product-link'
						);
						return;
					}
					productData.variation_attributes = {};
					...
					if (productData.has_variations) {
						$.ajax({
							...
							error(jqXHR, textStatus) {
								log(
									'AJAX error adding product to compare:',
									textStatus
								);
								//_this.addProductToCompare(productData, $(e.target));
								//_this.syncVariationAttributes();
		...
		$(document).on(
			'click',
			'.moreconvert-compare-section .moreconvert-compare-display-popup-button',
			function (e) {
				e.preventDefault();
		...
		$(document).on(
			'click',
			'.moreconvert-compare-add-product-popup',
			function (e) {
				e.preventDefault();
				if (
					$(e.target).hasClass(
					...
					$(
						'.moreconvert-compare-add-product-popup-wrapper'
					).fadeOut();
				}
			}
		);
		...
		$(document).on(
			'click',
			'.moreconvert-compare-close-button',
			function (e) {
				e.preventDefault();
		...
		$(document).on(
			'submit',
			'.moreconvert-compare-search-form',
			function (e) {
				e.preventDefault();
			}
		);
		...
		$(document).on(
			'input',
			'.moreconvert-compare-search-form .moreconvert-compare-input',
			this.debounce(
				(function (_this) {
					return function () {
						const searchQuery = $(this).val().trim();
						const baseProductId = $(this)
							.closest('.moreconvert-compare-search-form')
							.find('.moreconvert-compare-base-product-id')
							.val();
						...
						$('.moreconvert-compare-products-section')
							.find('.moreconvert-compare-message')
							.remove();
		...
		$(document).on(
			'click',
			'.moreconvert-compare-remove-from-compare',
			(function (_this) {
				return function (e) {
					e.preventDefault();
					const productId = $(this).data('product-id');
					_this.allCompareProducts = _this.allCompareProducts.filter(
						(p) => p.id !== productId
					);
					_this.compareProducts = _this.allCompareProducts.slice(
						0,
						_this.getMaxDisplayed()
					);
					_this.updateCompareColumns();
					_this.syncVariationAttributes();
					const baseProductId = $(
						'.moreconvert-compare-modal-content .moreconvert-compare-base-product-id'
					).val();
					_this.fetchSuggestedProducts(baseProductId);
				};
			})(this)
		...
		$(document).on(
			'click',
			'.moreconvert-compare-close-compare',
			(function (_this) {
				return function (e) {
					e.preventDefault();
	...
	CompareManager.prototype.syncVariationAttributes = function () {
		$.each(this.compareProducts, function (index, product) {
			const attrKeys = Object.keys(product.variation_attributes || []);
			if (attrKeys) {
				attrKeys.forEach((k) => {
					const key = 'attribute_' + k;
					const currentValue = product.selected_attributes[key] || '';
					$(
						`.moreconvert-compare-variation-index-${index} [data-attribute_name="${key}"]`
					)
						.val(currentValue)
						.trigger('change');
				});
	 ...
	 */
	CompareManager.prototype.processProductData = function (
		productData,
		responseData
	) {
		const parentFields = {
		...
		productData.variation_attributes =
			responseData.variation_attributes || {};
		...
		const selectedVar = this.findMatchingVariation(
		...
		if (selectedVar) {
			...
			productData.image =
				selectedVar.image_thumbnail || productData.parent_image;
			productData.price =
				selectedVar.price_html || productData.parent_price_html;
			productData.add_to_cart_url =
				selectedVar.add_to_cart_url ||
				productData.parent_add_to_cart_url;
		} else {
	...
	CompareManager.prototype.initializeComparison = function (productData) {
		if (!productData || !productData.id) {
			log('Invalid product data for comparison');
			return;
		}
		...
		this.compareProducts = this.allCompareProducts.slice(
			0,
			this.getMaxDisplayed()
		);
	 ...
	 */
	CompareManager.prototype.addProductToCompare = function (
		productData,
		$button
	) {
		if (!productData || !productData.id) {
			log('Invalid product data for adding to compare');
			return;
		}
		if (this.allCompareProducts.some((p) => p.id === productData.id)) {
			// replace productData with old
			const index = this.allCompareProducts.findIndex(
		...
		const loadedCount = $('.moreconvert-compare-products-grid').children()
			.length;
	 ...
	 * @param {Object} chosenAttributes The chosen attributes.
	 * @return {Object|null} Matching variation or null.
	 */
	CompareManager.prototype.findMatchingVariation = function (
		variations,
		chosenAttributes
	) {
		if (
		...
		return variations.find((v) => {
			return Object.keys(chosenAttributes).every((key) => {
				return (
					chosenAttributes[key] === '' ||
					v.attributes[key] === chosenAttributes[key] ||
					v.attributes[key] === ''
				);
			});
		});
	 ...
	 * @return {Object|null} Matching variation or null.
	 */
	CompareManager.prototype.findMatchingVariationID = function (
		variations,
		variationId
	) {
		if (!variations || !variationId || variationId.length === 0) {
			return null;
		}
	...
	CompareManager.prototype.updateAvailableOptions = function (index) {
		...
		const $card = $(
			`.moreconvert-compare-product-card:nth-child(${index + 1})`
		);
		if (selectedVar) {
			...
			product.add_to_cart_url =
				selectedVar.add_to_cart_url || product.parent_add_to_cart_url;
		...
		$card
			.find('.moreconvert-compare-product-image')
		...
		this.compareProducts[index].attributes = selectedVar
			? selectedVar.formatted_attributes
			: product.attributes;
		this.updateCompareColumns();
	...
	CompareManager.prototype.updateCompareColumns = function () {
		...
		$('.moreconvert-compare-base-product-id').val(
			this.compareProducts[0].id
		);

		$.each(this.compareProducts, function (index, product) {
			const $cardTemplate = wp.template(
				'moreconvert-compare-product-card-template'
			);
			const data = {
				product,
				index,
		...
		$('.moreconvert-compare-variation-selector .variations_form').each(
			(function (_this) {
				return function () {
					const $form = $(this);
					try {
						...
						log('Error initializing variation form:', e);
					}
					$form.on(
						'change',
						'select',
						(function (__this) {
							return function () {
								const $select = $(this);
								const attrKey = $select.data('attribute_name');
								...
								const index = $select
									.closest(
										'.moreconvert-compare-variation-selector'
									)
									.data('index');
								if (__this.compareProducts[index]) {
									__this.compareProducts[
										index
									].selected_attributes[attrKey] = value;
									if (value !== '') {
										__this.compareProducts[
											index
										].selected_labels[attrKey] = label;
									}
					...
					$form.on(
						...
						(function (__this) {
							return function (e, variation) {
								const index = $(this)
									.closest(
										'.moreconvert-compare-product-card'
									)
									.data('index');
								if (__this.compareProducts[index]) {
									__this.compareProducts[
		...
		if (
			...
			const $buttonTemplate = wp.template(
				'moreconvert-compare-add-button-template'
			);
			const data = {
				select_product_text:
					McCompare.texts.select_product || 'Select a product',
		...
		if (
			...
			for (
				...
				$grid.append(
					'<div class="moreconvert-compare-product-card moreconvert-compare-empty"></div>'
				);
			}
		}
		...
		this.compareProducts.forEach((product) => {
			(product.attributes || []).forEach((attr) => {
				...
				if (
					!(attr.key in attrOrders) ||
					(attr.order && attr.order < attrOrders[attr.key])
				) {
		...
		const sortedKeys = Object.keys(attrOrders).sort(
			(a, b) => attrOrders[a] - attrOrders[b]
		);
		const attributes = {};
		...
		this.compareProducts.forEach((product, index) => {
			(product.attributes || []).forEach((attr) => {
				attributes[attr.key][index] =
					attr.value ||
					(product.selected_labels &&
					product.selected_labels['attribute_' + attr.key]
						? product.selected_labels['attribute_' + attr.key]
						: '');
			});
		});
		const attributeTemplate = wp.template(
			'moreconvert-compare-add-attribute-template'
		);
		$.each(
			attributes,
			function (attrKey, values) {
	 ...
	 */
	CompareManager.prototype.loadProducts = function (
		baseProductId,
		isSearch = false,
		append = false
	) {
			...
			success: (function (_this) {
				return function (response) {
					...
					if (response.success && response.data.products) {
						...
						if (_this.observer) {
							const previousLastCard = $(
								'.moreconvert-compare-products-grid .moreconvert-compare-product-card:last-child'
							)[0];
							if (previousLastCard) {
						...
						$.each(
							...
							function (index, product) {
								if (
									!_this.allCompareProducts.some(
										(p) => p.id === product.id
									)
								) {
									const $itemTemplate = wp.template(
										'moreconvert-compare-item-template'
									);
									const data = {
						...
						if (
							response.data.products.length > 0 &&
							loadedCount < _this.totalProducts
						) {
							const $lastCard = $(
								'.moreconvert-compare-products-grid .moreconvert-compare-product-card:last-child'
							)[0];
	...
	CompareManager.prototype.updateProductCount = function (loaded) {
		const text = (
			...
			.replace('{totalProducts}', this.totalProducts);
	...
	$(function () {
		try {
			...
			log('Error initializing CompareManager:', e);
		}
	});
})(jQuery, document);
