 ...
 *
 * Version: 3.0.4 this version edited for this plugin
...
(function ($, undef) {
	...
	$.widget('a8c.iris', $.a8c.iris, {
		 ...
		 * @access private
		 *
		 * @param {Object|*} color - The color instance if not defined return the cuurent color.
		 *
		...
		_create() {
			try {
				...
				// Remove square href attribute because scroll to top of body on click it.
		 ...
		 *
		 * @param {jQuery} input - The input element to bind listeners to.
		 * @since 3.0.0
		 * @access private
		 *
		 * @return {void}
		 ...
		 * @access private
		 *
		 * @param {boolean} reset Set to True for recreate the controls sizes.
		 *
		 * @return {void}
		 */
		_dimensions(reset) {
			...
			if (this.alphaOptions.alphaEnabled) {
				...
				let stripWidth, stripMargin, totalWidth;
		...
		_setOption(key, value) {
			...
			if ('color' === key && self.alphaOptions.alphaEnabled) {
				...
				value = '' + value;
				const newColor = new Color(value).setHSpace(self.options.mode);
